...
수업 카드의 `학부모 보고 문자 만들기`를 누르면 적어 둔 **과목 · 진도 · 숙제**가 문장으로 조립된다.
예정 수업에서는 대신 당일 안내 문자가 나온다. 보내기 전에 그대로 고쳐 쓸 수 있고,
`보내기`를 누르면 문자·카톡 중에 고르게 된다(웹에서 공유가 막혀 있으면 클립보드로 복사한다).

문장은 매번 다르게 굴리지 않는다 — 같은 형식으로 꾸준히 가는 편이 받는 쪽이 읽기 편하다.
**적지 않은 항목은 줄째로 빠진다.** 무엇을 안 적었는지 학부모가 알 필요는 없기 때문이다.

`잔액 안내 넣기`를 누르면 남은 수업료 한 줄이 붙는다. 모자랄 때만 입금을 청하고,
남았을 때는 알리기만 한다. 문장 규칙은 전부 `test/message.test.mjs`에 케이스로 고정돼 있다.

호칭(`어머님`/`아버님`)과 주로 가르치는 과목은 학생 정보에서 정한다.
이름 끝 받침에 따라 `민준이 어머님` / `지수 어머님`으로 조사가 갈린다.
...
src/message.js        학부모께 보낼 문자를 만든다. 화면을 모르는 순수 함수
src/MessageSheet.js   만든 문자를 보여주고 고쳐 쓴 뒤 내보낸다
src/SubjectInput.js   과목 입력 (친 글자로 좁힌다)
src/share.js          문자를 다른 앱으로 넘긴다 (웹은 share.web.js)
```
